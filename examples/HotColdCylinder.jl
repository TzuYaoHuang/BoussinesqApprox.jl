...
function hot_cold_circle(N;Ra=10^6,ε=0.001,Pr=100,mem=Array,T=Float32)
    ...
    ThermalSimulation(NN, (0,0), radius; U, Δt=0.05, ν, α, κ, g=(i,x,t)-> i==2 ? -g : zero(g), perdir=(1,),θ0=nothing,θb=(x,t)-> -sign(x[2]-N)*ΔT, body)
end

# using CUDA
sim = hot_cold_circle(96)#; mem=CuArray)
...
sim_gif!(sim; duration=20, step=0.08, f=(dat,sim)->copyto!(dat,sim.flow.θ), clims=(-1,1), cfill=:seismic, hidedecorations=true,
