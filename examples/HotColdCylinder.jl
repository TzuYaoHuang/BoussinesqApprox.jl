...
function hot_cold_circle(N;Ra=10^6,ε=0.001,Pr=100,mem=Array,T=Float32)
    NN = (N,N)
    radius = N÷8
    body = AutoBody((x,t)->√sum(abs2, x .- (N÷2,N÷4)) - radius) + AutoBody((x,t)->√sum(abs2, x .- (N÷2,3N÷4)) - radius)
    ...
    Simulation(NN, (0,0), radius; U, Δt=0.05, ν, κ, flow_ctor=ThermalFlow, g=(i,x,t)-> i==2 ? -g : zero(g), perdir=(1,),θ0=nothing,θb=(x)-> -sign(x[2]-N÷2)*ΔT, body)
end
...
sim_gif!(sim; duration=40, step=0.08, field=sim->sim.flow.θ, clims=(-1,1), cfill=:seismic,
              plotbody=true, fname=joinpath(@__DIR__,"HotColdCylinder.gif"))